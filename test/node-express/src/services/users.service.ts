...
class UsersService extends Service{
    ...
    find = async( options: any ) => {
        ...
        if ( options ) {
            ...
            [items, error] = await handleAsync(getRepository(this.entity).find( {relations: ["address"]} ));
        }
    ...
    findOne = async( id: string ) => {
        ...
        let [item, error] = await handleAsync(getRepository(this.entity).findOne( id, {relations: ["address"]} ));
    ...
    update = async( id: string, data:any ) => {
        ...
        let [updatedItem, error2] = await handleAsync(getRepository(this.entity).findOne(id, {relations: ["address"]}));
